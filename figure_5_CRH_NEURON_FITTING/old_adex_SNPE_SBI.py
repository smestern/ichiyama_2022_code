...
#%%
def adex_model(cm_i, taum_i, El_i, a_i, b_i, tau_a_i, Dt_i, Vt_i, VR_i, realC_i=None, record_v=False) -> [StateMonitor, SpikeMonitor]:
    '''
    ...
    G1 = NeuronGroup(N, eqs, threshold='v > Vcut', reset='v = VR; w += b', method='Euler', refractory=1*ms)
    #init:
    G1.v = El_i *mV
    G1.a = a_i * nS
    G1.b = b_i * pA
...
def generate_fi_curve(param_set, save=True) -> torch.tensor:
    ...
    if param_list.shape[0] > 12:
        cm_i, taum_i, El_i, a_i, Dga_i, tau_a_i, Dt_i, Vt_i, VR_i = np.hsplit(np.vstack(param_list), 9) #Splits the variables out of the array
    ...
    for i, sweep in enumerate(realC): #For each sweep
        ...
        voltage, spikes = adex_model(cm_i, taum_i, El_i, a_i, Dga_i, tau_a_i, Dt_i, Vt_i, VR_i, realC_i=sweep, record_v=True) #Run the adex model with the passed in params
        ...
        for p in np.arange(N): #For each neuron
                ...
                if len(pspikes) > 0: #If there are any spikes
                    ...
                    if len(spike_s) > 1: #If there is more than one spike
                        isi_full[p].append(compute_distro_mode(np.diff(spike_s), bin=5)) #compute the mode ISI
                    else:
...
_ = utils.pairplot(posterior_samples, 
                    fig_size=(15,15), labels=["Cm", "taum", "El", "A", "b", "tauw", "DeltaT", "Vt", "VR"])
N=1
