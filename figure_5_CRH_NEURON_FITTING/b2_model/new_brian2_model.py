...
class brian2_model(object):
    ...
    def opt_full_mse(self, param_dict):
                ...
                if len(pspikes) > 0:
                    ...
                    if sweep in self.spikeSweep:
                        if len(spike_s)< 1: 
                            ...
                            try:
                                temp_dist = compute_spike_dist_euc(np.hstack((np.log10(spike_s[0]*1000), np.diff(spike_s)*1000)), np.hstack((np.log10(self.spike_times[self.activeSweep][0]*1000), np.diff(self.spike_times[self.activeSweep])*1000)))
                            except:
                                temp_dist = 99
        ...
        real_FI = self.realFI
        log_sim = np.nan_to_num(np.log10(spikes_return), posinf=0, neginf=0)
        log_real = np.nan_to_num(np.log10(real_FI), posinf=0, neginf=0)
        unit_wise_isi_e = np.apply_along_axis(compute_mse,1,np.nan_to_num(np.log10(isi_return), posinf=0, neginf=0),np.nan_to_num(np.log10(self.realISI), posinf=0, neginf=0))
        unit_wise_error = np.apply_along_axis(compute_mse,1,log_sim,log_real)
        error_fi = unit_wise_isi_e + unit_wise_error

        
        return 0, error_t, error_fi, error_s
    ...
    def opt_FI(self,param_dict=None):
        ...
        log_real = np.nan_to_num(np.log10(real_FI), posinf=0, neginf=0)
        unit_wise_isi_e = np.apply_along_axis(compute_mse,1,np.nan_to_num(np.log10(unit_wise_isi), posinf=0, neginf=0),np.nan_to_num(np.log10(self.realISI), posinf=0, neginf=0))
        unit_wise_error = np.apply_along_axis(compute_mse,1,log_sim,log_real)
