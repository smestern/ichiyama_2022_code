...
class brian2_model(object):
    ...
    def opt_FI(self,param_dict=None):
        ...
        unit_wise_isi_e = np.apply_along_axis(compute_mse,1,np.nan_to_num(np.log10(unit_wise_isi+1), posinf=0, neginf=0),np.nan_to_num(np.log10(self.realISI+1), posinf=0, neginf=0))
        unit_wise_error = np.apply_along_axis(compute_mse,1,unit_wise_FI,real_FI)
